...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('PDF Export requested for user:', user.id);
    ...
    if (timeEntries.length === 0) {
      console.log('No data found, creating "No Data" PDF');
      ...
      // Add title
      doc.setFontSize(20);
      doc.text('Time Entries Report', 20, 30);
      ...
      doc.setFontSize(10);
      doc.text('Try adjusting your date range or check if you have any time entries.', 20, 85);
      
      const pdfBuffer = doc.output('arraybuffer');
      const filename = `time_entries_${startDate || 'all'}_to_${endDate || 'all'}_no_data.pdf`;
    ...
    autoTable(doc, {
      ...
      head: [['Date', 'Task', 'Description', 'Duration']],
      body: tableData,
      theme: 'striped',
      styles: {
        fontSize: 10,
    ...
    const pdfBuffer = doc.output('arraybuffer');
    const filename = `time_entries_${startDate || 'all'}_to_${endDate || 'all'}.pdf`;
